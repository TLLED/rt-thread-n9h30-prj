...
uint8_t txdat[8]={0,0,0,0,0,0,0,0};
 ...
 void main_motor_setvalue(uint8_t cr, uint8_t vt, uint8_t sp)
  {
     ...
     lv_label_set_text_fmt(motor_cur_label,    "Current --%dA",cr);
     lv_label_set_text_fmt(motor_volt_label,   "Voltage -- %dV",vt);
     lv_label_set_text_fmt(motor_spd_label,    "Speed -- %dkm/h",sp);
  }
  ...
  void main_canrx_setvalue(uint8_t *cx)
  {
      ...
      lv_label_set_text_fmt(can_rxdat,"%02x %02x %02x %02x %02x %02x %02x %02x",cx[0],cx[1],cx[2],cx[3],cx[4],cx[5],cx[6],cx[7]);
  ...
  void main_canpar_setvalue(uint8_t *cp)
  {
      ...
      sprintf((char*)temp_data, "%.1f", cp[0]*0.1);
      lv_label_set_text_fmt(can_par0,"par1: %sm",temp_data);

      sprintf((char*)temp_data, "%.1f", cp[1]*0.1);
      lv_label_set_text_fmt(can_par1,"par2: %sm",temp_data);

      sprintf((char*)temp_data, "%.1f", cp[2]*0.1);
      lv_label_set_text_fmt(can_par2,"par3: %sm",temp_data);

      sprintf((char*)temp_data, "%.1f", cp[3]*0.1);
      lv_label_set_text_fmt(can_par3,"par4: %sm",temp_data);
